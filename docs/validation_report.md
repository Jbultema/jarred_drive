...
Validated on 2026-08-20 using Python 3.12.6, Poetry 1.8.3, Xcode 16.2, and the checked-in lock file.

## Software evidence

- `make check` passed: Ruff, Black, mypy across 26 source files, 44 pytest cases, six native
...
- Streamlit AppTest exercised all ten pages against all four scenarios: Devices / Sync, Flight Deck, Launch
...
- The SwiftUI target compiled successfully for the generic iOS Simulator destination using Xcode 16.2. HTTP
  integration tests exercised the desktop ESP emulator, including Range resume and authenticated
  acknowledgement. A simulator runtime was not visible to the command-line environment, so interactive UI
  launch remains a local Xcode step rather than validation evidence here.
...
- The committed synthetic package contains 55 artifacts across four deterministic coupled scenarios,
...
| FOIL_004 | Commissioning repeat | 87.5% | 12.5% | 0 | 7 | 62.2% | 133.5 Wh | 24.6 C | 0.6 C | 71 s |
